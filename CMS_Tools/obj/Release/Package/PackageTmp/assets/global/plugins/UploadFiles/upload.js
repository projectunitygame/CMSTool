$(document).ready(function () {
    ...
    *****************************/
    ...
    dropzone3 = $('#droparea3'),
    uploadBtn3 = $('#uploadbtn3'),
    defaultUploadBtn3 = $('#upload3');

    div = $('#tblImages tbody');
    div1 = $('#tblImages1 tbody');
    div2 = $('#tblImages2 tbody');
    div3 = $('#tblImages3 tbody');
    /*****************************
    Events Handler
    *****************************/
    ...
    dropzone.on('drop', function (e) {
        ...
        processFiles(files, div, dropzone);
        return false;
    });
    ...
    dropzone1.on('drop', function (e) {
        ...
        //retrieve uploaded files data
        var files = e.originalEvent.dataTransfer.files;
        processFiles(files, div1, dropzone1);
        return false;
    });
    uploadBtn1.on('click', function (e) {
        e.stopPropagation();
        e.preventDefault();
        //trigger default file upload button
        defaultUploadBtn1.click();
    });
    defaultUploadBtn1.on('change', function () {
        //retrieve selected uploaded files data
        var files = $(this)[0].files;
        processFiles(files, div1, dropzone1);
        return false;
    ...
    dropzone2.on('drop', function (e) {
        ...
        //retrieve uploaded files data
        var files = e.originalEvent.dataTransfer.files;
        processFiles(files, div2, dropzone2);
        return false;
    });
    ...
    dropzone3.on('drop', function (e) {
        ...
        //retrieve uploaded files data
        var files = e.originalEvent.dataTransfer.files;
        processFiles(files, div3, dropzone3);
        return false;
    });
    uploadBtn3.on('click', function (e) {
        e.stopPropagation();
        e.preventDefault();
        //trigger default file upload button
        defaultUploadBtn3.click();
    });
    ...
    *****************************/
    ...
    Process FileList 
    *****************************/
    var processFiles = function (files, div, _dropzone) {
        if (files && typeof FileReader !== "undefined") {
            //process each files only if browser is supported
            for (var i = 0; i < files.length; i++) {
                readFile(files[i], div, _dropzone);
            }
        } else {
    ...
    *****************************/
    var readFile = function (file, div, _dropzone) {
        if ((/image/i).test(file.type)) {
            ...
            reader.onload = function (e) {
				...
				.load(function () {
				    ...
				    uploadToServer(file, newimageurl, div, _dropzone);
				})
            ...
            _dropzone.find(".err").html("");
        } else {
            //some message for wrong file format
            _dropzone.find(".err").html('*Selected file format not supported!');
        }
    }
    ...
    *****************************/
    ...
    Draw Image Preview
    *****************************/
    var createPreview = function (file, newURL, realLink) {
    ...
    var createPreviewReal = function (data,div) {
        //populate jQuery Template binding object
        //var imageObj = {};
        //imageObj.filePath = data.urlPhone;
        //imageObj.fileName = data.fileName;
        //imageObj.fileOriSize = data.sizeOld;
        //imageObj.fileUploadSizeWeb = data.sizeWeb;
        //imageObj.fileUploadSizeTablet = data.sizeTablet;
        //imageObj.fileUploadSizePhone = data.sizePhone;
        //imageObj.urlWeb = data.urlWeb;
        //imageObj.urlPhone = data.urlPhone;
        //imageObj.urlTable = data.urlTablet;

        //append new image through jQuery Template
        div.append("<tr id='" + data.id + "'>" +
                            "<td><img class='urlImg' data-fileSize='" + data.sizeWeb +
                                 "' data-fileName='" + data.fileName +
                                 "' data-urlweb='" + data.urlWeb +
                                 "' data-urltablet='" + data.urlTablet +
                                 "' data-urlphone='" + data.urlPhone +
                                 "' src='" + data.urlPhone + "' style='width: 50px; border: 1px solid gray;margin:5px 10px 5px 0px;max-height:65px;'/></td>" +
                            "<td><label title='" + data.fileName + "'>" + data.fileName + "<label></td>" +
                            "<td>" + data.sizeWeb + "</td>" +
                            "<td class='colDel'><button type='button' class='btn red delete' onclick='$(\"#" + data.id + "\").remove();'>" +
    ...
    ****************************/
    var uploadToServer = function (oldFile, newimageurl, div, _dropzone) {
        // prepare FormData
        ...
        $.ajax({
            url: 'http://picture.mysystem.vn/UploadPicture.ashx?token=' + $('#tokenUpload').val(),
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            beforeSend: function () {
                _dropzone.find('.loading').css("display", "block");
            },
            success: function (data) {
                if (data.status == 0) {
                    createPreviewReal(data.data, div);
                }
                else
                    alert(data.msg);
            },
            complete: function () {
                _dropzone.find('.loading').css("display", "none");
            }
        });
    }
